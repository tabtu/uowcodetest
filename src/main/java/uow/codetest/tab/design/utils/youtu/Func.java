package uow.codetest.tab.design.utils.youtu;
...
public class Func {
	...
	public JSONObject NewPerson(byte[] image, String person_id, List<String> group_ids)
			throws IOException, JSONException, KeyManagementException, NoSuchAlgorithmException {
		JSONObject data = new JSONObject();

		data.put("imageA", Base64Util.encode(image));
		data.put("person_id", person_id);
		data.put("group_ids", group_ids);
